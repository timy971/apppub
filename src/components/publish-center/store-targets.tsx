...
export function StoreTargetsCard({ project, status }: { project: Project; status: ProjectStatus }) {
  ...
  return (
    <Card className="p-6 shadow-soft">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-base font-semibold">Destination de publication</h2>
      </div>
      <div>
        <PlatformTile
          project={project}
...
function PlatformTile({
  ...
  return (
    <div className={"rounded-xl border p-4 " + (muted ? "bg-muted/30" : "bg-background")}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground truncate">{readiness.summary}</div>
          </div>
        </div>
        <Button asChild size="sm" variant="ghost">
          <Link to="/projects/$id" params={{ id: project.id }} search={{ tab: "publishing" }}>
            Configurer
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 text-xs">
        {readiness.configLines.map((line) => (
          <div key={line.label} className="min-w-0">
            <dt className="uppercase tracking-wide text-muted-foreground">{line.label}</dt>
            <dd
              className={
                "truncate mt-0.5 " +
        ...
        <div className="min-w-0">
          ...
          <dd className="truncate mt-0.5 font-medium">{labelTrack(readiness.defaultTrack)}</dd>
        </div>
        <div className="min-w-0">
          <dt className="uppercase tracking-wide text-muted-foreground">Langue</dt>
          <dd className="truncate mt-0.5 font-medium">{readiness.primaryLanguage ?? "fr-FR"}</dd>
        </div>
      </dl>
...
function PlatformBadge({ severity, present }: { severity: PubSeverity; present: boolean }) {
  if (!present) {
    return (
  ...
  const label = severity === "error" ? "Bloqué" : severity === "warn" ? "À vérifier" : "Prêt";
